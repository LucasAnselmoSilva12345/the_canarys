...
(function ($) {
  ...
  $(".banner-slider-active-with-navigation").slick({
    ...
    arrows: true,
    prevArrow:
      '<span class="cr-slider-nav cr-slider-nav-left"><i class="icofont icofont-simple-left"></i></span>',
    nextArrow:
      '<span class="cr-slider-nav cr-slider-nav-right"><i class="icofont icofont-simple-right"></i></span>',
  });
  ...
  /* Mobile Menu Minimal */
  $(
    ".header-minimal-1 nav.menu, .header-minimal-2 nav.menu, .header-minimal-3 nav.menu"
  ).meanmenu({
    meanMenuClose: '<i class="icofont icofont-close"></i>',
    meanMenuCloseSize: "18px",
    meanScreenWidth: "991",
  ...
  if ($("#sidebar-instagram-feed").length) {
    var userFeed = new Instafeed({
      ...
      limit: 6,
      template:
        '<li><a href="{{link}}" target="_new"> <img src="{{image}}" /><ul class="likes-comments"><li><i class="icofont icofont-heart"></i><span>{{likes}}</span></li><li><i class="icofont icofont-speech-comments"></i><span>{{comments}}</span></li></ul></a></li>',
    });
    userFeed.run();
  }
